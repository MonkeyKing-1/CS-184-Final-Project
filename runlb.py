...
def sample(theta, env : gym.Env, N, lb):
    ...
    for i in range(N):
        ...
        len = 0
        while True:
            phis = utils.extract_lb_features(observation, lb)
...
def train(env, N, T, lb, delta, lamb=1e-3):
    """
    ...
    for i in range(T):
        print(i)
        grads, rewards = sample(theta, env, N, lb)
        rsum = 0
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 2:
        ...
        if len(sys.argv) >= 3:
            ...
            assert False
    gym.register(
        id='InfLbInvestorGameEnv-v0',
        entry_point='environments.inflbenvironment:InfLinInvestorGameEnv', 
        kwargs={'gamma': None, 'lb': None} 
    )
    

    env = gym.make('InfLbInvestorGameEnv-v0', gamma = 1/gamma, lb = lb)
    np.random.seed(1234)
    theta, episode_rewards = train(env, N=1000, T=100, lb = lb, delta=1e-2)
    episode_rewards = np.array(episode_rewards)
    baseline = np.zeros_like(episode_rewards)
    f = open("params/inflb-" + str(lb) + ".txt", "w")
    for i in theta:
        f.write(str(i[0]))
        f.write(" ")
    f.close()
