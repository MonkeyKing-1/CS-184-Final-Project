...
def infinite_gen(N, model, func):
    theta = np.loadtxt("params/inflinrew-" + model + ".txt")
...
def piecewise_log_lin(N):
    theta_lin = np.loadtxt("params/inflinrew-lin.txt")
    theta_lin = theta_lin.reshape((len(theta_lin), 1))
    theta_log = np.loadtxt("params/inflinrew-log.txt")
...
if __name__ == '__main__':
    ...
    if model == "sqrt":
        infinite_gen(num_runs, model, partial(pow_util, 1/2))
    if model == "cbrt":
        infinite_gen(num_runs, model, partial(pow_util, 1/3))
    if model == "lin":
        lin_scores = infinite_gen(num_runs, model, partial(pow_util, 1))
        log_scores = infinite_gen(num_runs, "log", log_util)
        # log_scores = cheese_strat(num_runs, 0.8)
        ...
        other_scores = [sqrt_scores, cbrt_scores, log_scores, inv_scores]
    ...
    if model == "log":
        log_scores = infinite_gen(num_runs, model, log_util)
        # log_scores = cheese_strat(num_runs, 0.8)
        sqrt_scores = infinite_gen(num_runs, "sqrt", partial(pow_util, 1/2))
        cbrt_scores = infinite_gen(num_runs, "cbrt", partial(pow_util, 1/3))
        lin_scores = infinite_gen(num_runs, "lin", partial(pow_util, 1))
