...
class FinLogInvestorGameEnv(gym.Env):
    def __init__(self, max_rounds):
        super(FinLogInvestorGameEnv, self).__init__()
    ...
    def step(self, action):
        ...
        if action == 1:
            ...
            if samp > self.prob:
                reward = log(self.cur_pos + self.downside) - log(self.cur_pos)
            else:
                reward = log(self.upside + self.cur_pos) - log(self.cur_pos)
        else:
            reward = 0
        self.cur_pos *= exp(reward)
        self.round += 1
